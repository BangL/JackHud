...
function MissionAssetsManager:update_buy_all_assets_asset_cost()
	if self._tweak_data.buy_all_assets and not self:mission_has_preplanning() and JackHUD:GetOption("enable_buy_all_assets") then
...
function MissionAssetsManager:insert_buy_all_assets_asset()
	if self._tweak_data.gage_assignment and not self:mission_has_preplanning() and JackHUD:GetOption("enable_buy_all_assets") then
...
function MissionAssetsManager:check_all_assets()
	if game_state_machine and not self:mission_has_preplanning() and JackHUD:GetOption("enable_buy_all_assets") then
		for _, asset in ipairs(self._global.assets) do
			if self:asset_is_buyable(asset) then
				return
			end

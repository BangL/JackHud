...
function WeaponLaser:update(unit, t, dt, ...)
	...
	if suffix then
		if JackHUD:GetOption("enable_laser_options" .. suffix) then
			local r, g, b = JackHUD:GetOption("laser_color_r" .. suffix), JackHUD:GetOption("laser_color_g" .. suffix), JackHUD:GetOption("laser_color_b" .. suffix)
			if JackHUD:GetOption("laser_color_rainbow" .. suffix) then
				r, g, b = math.sin(135 * t + 0) / 2 + 0.5, math.sin(140 * t + 60) / 2 + 0.5, math.sin(145 * t + 120) / 2 + 0.5
			end
			self._themes[theme] = {
				light = Color(r, g, b) * JackHUD:GetOption("laser_light" .. suffix),
				glow = Color(r, g, b) * JackHUD:GetOption("laser_glow" .. suffix),
				brush = Color(JackHUD:GetOption("laser_color_a" .. suffix), r, g, b)
			}
		else
			self._themes[theme] = self._default_themes[theme]
		end
	else
		log("JackHUD - Ignoring unknown laser theme: \"" .. theme .. "\".")
	end
